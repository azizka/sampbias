...
#   print(i)
  ...
  out <- calculate_bias(occ, res = res, buffer = 0.5, restrict_sample = borneo)

  save(out, file = paste("empirical_analyses/simulations/simulation_result_", ID, ".rda", sep = ""))
  ...
  # write to disk
  seed <- .Random.seed
  save(seed, file = paste("empirical_analyses/simulations/simulation_seed_", ID, ".rda", sep = ""))
  write_csv(out, paste("empirical_analyses/simulations/simulation_biasweights", ID, ".csv", sep = ""))
