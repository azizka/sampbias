...
#Installing sampbias
To install the *sampbias* R-package, you can either download the latest build version from the gitHub folder:

```{r}
install.packages("sampbias_0.1.0.zip", repos = NULL)
library(sampbias)
```

or use the `devtools` package to install directly from R:
...
occ <- read.csv("data/mammals_borneo.csv", sep = "\t")
head(occ)
```
...
cit <- readShapeSpatial("Borneo_major_cities.shp")
roa <- readShapeSpatial("Borneo_major_roads.shp")
...
|convexhull|logical. If TRUE, the empirical distribution (and the output maps) is restricted to cells within a convex hull polygon around \code{x}. If FALSE a rectangle around \code{x} is used.  Default = FALSE.|
...
```{r, out.width = "7in", echo = FALSE, fig.cap = "XXX.", eval = TRUE}
knitr::include_graphics(path = "img/visualization_rpackage_application_githubtutorial.jpg")
```



The plot function creates a set of publication level plots using the `ggplot2` package, including the number of occurrence per gridcell, the number of species per gridcell, a visualization of the bias for sources provided via the gazetteers in `gaz` and the average bias over all sources. To write the plots to a file, use `pdf` and `dev.off`:
