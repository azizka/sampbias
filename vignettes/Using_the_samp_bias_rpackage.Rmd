...
# Installing sampbias
To install the *sampbias*, you can use the `devtools` package:
...
The example files for this tutorial are provided in the with the package. You can find descriptions and help for all functions by typing `?Functionname`, for instance `?calculate_bias`.
...
See [here](https://cran.r-project.org/web/packages/sp/vignettes/intro_sp.pdf) and `?SpatialPointsDataFrame` or `?SpatialLinesDataFrame` on how to create a Spatial Object from tables of coordinates.
...
In addition to the input from above, `calculate_bias` offers a set of options to customize analyses, of which the most important ones are shown in Table 1. See `?calculate_bias` for a detailed description of all options, and the section "Changing default settings" of this tutorial for a description and examples for the "restricted sample" and "inp_raster" options.
...
|Object|Description|
|---|---|
|summa|A list of summary statistics for the sampbias analyses, including the total number of occurrence points in x, the total number of species in x, the extent of the output rasters as well as the settings for res, binsize, and restrict_area used in the analyses.|
...
The package includes summary and plot methods for an easy exploration of the results. The plot method generates a boxplot of the posterior estimates of the weights for each biasing factor.
...
```{r, fig.cap = "Sampling bias effect projected in space."}
...
```{r, fig.cap="Example of a customized study area which can be provided to the clacualte bias function via the restrict_sample option. Only grid cells within red area are included in the sampling bias calculation."}
data(area_example)
...
```{r, fig.cap="Example of a more complex customized study arearepresenting the Borneo montane rain forests ecoregion from Olson et al. (2001). Only grid cells within red area are included in the sampling bias calculation."}
...
## Equal area grid
By default *sampbias* uses a lat/lon grid. This is a reasonable approximation for local and regional scales. However, since the area of the cells in such a raster differs with latitude for large-scale studies an equal area grid---for instance using a Behrmann projection---is a better choice. When using an equal area grid, it is necessary for the input occurrence records as well as the gazetteer files to share the same projection and coordinate reference system.
